## Clipboard-Saver 1.8.0

### Downloads

| File | Use it when |
|---|---|
| `Clipboard-Saver_1.8.0_x64-setup.exe` | Installer — choose **all users** (admin) or **current user only**; upgrades replace the previous version automatically, your data is kept |
...
- **Saved player state**: volume, mute and the loop / play-once choice are remembered per snippet and restored on the next start
...
- Saving a snippet shows an error message if it fails instead of failing silently
