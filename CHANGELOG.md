...
- **Saved player state**: volume, mute and the loop / play-once choice are
  remembered per snippet and restored on the next start
...
- Saving a snippet shows an error message if it fails instead of failing
  silently
...
- Upgrading over an existing installation no longer asks about the previous
  version — it is removed automatically (your snippets and settings are kept)
...
### Added
- **Image tiles**: the image button next to the input saves a picture from
  the clipboard (or via file dialog) — clicking the tile copies the image,
  ready to paste anywhere
- **Icon images**: any text snippet can show a picture on its tile instead of
...
### Added
- **Text library**: new list button in the header shows every saved snippet
...
- Hovering a tile shows the stored text (what gets copied) in the tooltip

### Fixed
- Wrong text size right after starting on monitors with display scaling
- Tile text no longer shifts while dragging a tile — the drag ghost now
...
- Deleting a snippet always asks for a second confirmation
- The overflow tray is gone — unplaced snippets live in the text library
...
- **Floating button menu**: right-click a floating pill for size presets
  (S / M / L), editing the snippet, or removing the pill
...
- Snippet grid with free placement, multiple views, per-snippet colors,
